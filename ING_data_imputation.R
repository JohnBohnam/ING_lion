...
library(ggplot2)
#data inspection####
data <- read.csv(file = "development_sample.csv")
data %>%  attach() 
...
maciek <- read.csv(file = "dla_daniela.csv")
...
data_debug <- data #just for debugging purposes
rm(maciek)
#NA check (just inspection, not needed in python for imputation)
summary(data)
...
#missing at random check (just inspection, not needed in python for imputation)
...
rm(x)
# data dependency visualisantion (just inspection, not needed in python for imputation)
